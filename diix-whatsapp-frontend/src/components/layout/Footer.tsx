...
export function Footer({ 
  ...
  return (
    <footer
      className={cn(
        "glass-panel border-t border-white/10 px-4 sm:px-6 py-3 sm:py-6 mt-auto",
        className
      )}
    >
          ...
          <div className="flex flex-col items-center gap-2 text-center">
            ...
            >
              <p className="text-xs text-text-muted">
                © {currentYear} DIIX WhatsApp
              </p>
            </motion.div>
            
            {!isSmallMobile && showVersion && (
              <motion.span
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 0.3 }}
                className="inline-flex items-center px-2 py-1 rounded-full bg-accent-primary/10 text-accent-primary text-xs font-medium"
              >
                v{APP_VERSION}
              </motion.span>
            )}
